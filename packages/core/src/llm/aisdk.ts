  ...
  async answer(
    ...
    const briefingExtras = [
      taskTools
        ...
        : "",
      taskTools
        ? "VOICE NOTES: a message that begins 'voice note transcript from …' is a transcribed voice note — treat it EXACTLY as if the user had typed it and reply the same way. Then decide, on merit, whether it is worth keeping: if it carries substantive content worth remembering later (a decision, idea, plan, commitment, or fact you'd want in the vault), file it with capture_note; if it is ephemeral (a quick question, chit-chat, or a one-off instruction you have already acted on), just reply and do NOT file it. The transcript already lives in the conversation either way. Keeping is silent and on-merit — never frame a voice note as 'queued for filing/digestion'."
        : "",
      driveTools
