  ...
  async answer(
    ...
    const taskToolSet = taskTools
      ? {
          ...
          approve_queue: tool({
            description:
              "Promote specific backlog issues from status:proposed to status:queued so the monitor executes them. THIS IS THE ONLY TOOL THAT CAN SET status:queued, and it is the ONLY way a ticket ever runs — until you call it the ticket sits at status:proposed and the runner ignores it. Call it when the human has EXPLICITLY approved execution: either by number (e.g. 'yes, queue #1 and #2', 'approve 51 and 53, run them'), OR when the human asked to create-and-queue a ticket this same turn — pass the number create_issue just returned. Never infer approval from a vague or general request. CRITICAL: do NOT tell the user a ticket is 'queued', 'queued for execution', 'running', or that the runner was 'poked'/'woken' unless this tool actually returned success for that number — there is NO separate tool to poke, wake, or notify the runner, so never claim you did; the monitor picks up queued tickets on its own within ~2 minutes. After queuing, tell the user exactly which issues were queued.",
            inputSchema: z.object({
              repo: z.string().nullable().describe("owner/repo target; null uses the configured vault/project repo"),
              issueNumbers: z
