...
const classificationSchema = z.object({
  ...
  topics: z.array(z.object({
    topic: z.string().describe("One independently maintainable proposition, not a whole project, branch or subject bundle"),
    ...
    })).max(24).describe("Source-qualified metadata for this topic's single proposition; empty when unavailable. Never bundle independently maintainable claims in this array: give each its own topic instead"),
    ...
    })).describe("Exact quotes overlapping the addressed code-provided passage; quotes may cross contiguous supplied neighbors. Use occurrence 0 for a unique match, otherwise the zero-based occurrence among matches overlapping that passage. Multiple ideas may share a passage; one idea may quote multiple passages. Empty only for legacy inputs without passage IDs."),
  ...
  })).min(1).describe("One topic per independently maintainable proposition, each with its own confidence, evidence and filing decision. Many topics may share the same destination page; include uncertain propositions too"),
...
export class AiSdkBrainLlm implements BrainLlm, TurnPlanCompiler {
  ...
  constructor(options: AiLlmOptions) {
    ...
    if (options.organizerReasoningEffort !== undefined) {
      if (!["openai", "openrouter"].includes(options.provider) || !["none", "low"].includes(options.organizerReasoningEffort)) {
  ...
  async classify(input: ClassifyInput): Promise<Classification> {
    ...
    try {
      ...
      system: [
        ...
        "Extract and route every substantive idea before choosing a disposition. Reconciliation alone decides ADD versus reinforcement versus conflict: you do not decide novelty. Repeated existing knowledge still needs a new source citation. Shortness, repetition, or unchanged plans are not reasons to skip a substantive idea. For known destinations reuse exact supplied existing page paths. A clearly new project may propose a valid new path under the existing create-page contract; clarify ambiguity and never raise confidence to force routing.",
        "- evidence_only: the capture contains no substantive proposition for organized memory. Keep its raw evidence. Never use this for substantive project facts, constraints, intentions, responsibilities, tentative ideas or reinforcement, even when already known.",
        "- append_compact_note: one durable proposition (including a qualified hypothesis or attributed unconfirmed report) belongs on an existing page and can be represented by one short cited update without rewriting the page.",
      ...
      tools: {
        ...(input.answerSupportContract ? {
          submit_memory_answer: tool({
            description: "Finish this memory answer by selecting actual answerSupports IDs and allowed modes. Select all requested subjects and necessary source qualifications. For a whole-note summary choose one source_summary handle and write one coherent summaryText of at most 1200 characters covering every explicitly requested facet and qualifier, avoiding repetitive filler. Read completeness does not validate semantic coverage or correctness. For other paraphrases use parent paragraph support, never sentence-granularity handles; summaryText:null explicitly selects verbatim excerpts and does not summarize; always include this field. Use complete relevant supports and preserve attribution, options, uncertainty, conditions and ambiguous numbers; other modes retain canonical wording. For a requested assessment, include analysisText grounded in these selected premises; null for source-only answers. No final prose outside this tool is accepted. This terminal tool ends the current turn without another completion.",
