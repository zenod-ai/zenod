    ...
    try {
      const result = await generateObject({model: this.model(this.classifyModelId), schema, maxOutputTokens: 4000,
        system: [
          ...
          "Cover every idea independently, even when ideas share sourceIds. sourcePartial on an idea means source evidence was omitted for budget, so do not claim the entire idea complete; omitted evidence remains pending. Batch ideas on this branch. Prefer one operation per atomic idea. Partial context cannot prove a statement absent; clarify when the selected current statements cannot support a safe decision.",
