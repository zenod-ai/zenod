    ...
    try {
      const result = await generateObject({model: this.model(this.classifyModelId), schema, maxOutputTokens: 4000,
        system: [
          ...
          "Use only provided ideaIds, sourceIds and targetId. Explicitly account for each IDEA, not just each source passage: several distinct thoughts can share a passage. The supplied source envelope is context, not text to copy wholesale. sourceQuote must be the shortest exact complete relevant proposition clause within that envelope, preserving actor, recipient, scope, negation, uncertainty and attribution; exclude unrelated adjacent claims or numbers. Keep sourceQuote within 1600 characters and statement within 800 characters. For ADD/SUPERSEDE/CONFLICT, statement is concise supported wording (at most800characters), retaining owner/time/negation/modality. For LINK_SOURCE leave statement null; add only the citation. For ADD use null targetId and a stable entity.attribute factKey only when the source expresses a durable current-state claim. LINK_SOURCE changes no prose. SUPERSEDE requires a nonempty statement containing ONLY the new current claim; never copy the rejected old claim or narrate preservation of history into statement. Prefer sourceQuote as the exact replacement clause and correctionQuote as the wider explicit correction. The host preserves the old statement and provenance automatically; do not emit a separate ADD for history. SUPERSEDE must reuse target's factKey when present (otherwise propose a stable entity.attribute key) and exact correctionQuote proving explicit intent; it may use a natural Spanish/English correction without repeating the old statement. Mere disagreement, hypothetical changes or uncertainty use CONFLICT/CLARIFY.",
