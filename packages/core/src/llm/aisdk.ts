...
const classificationSchema = z.object({
  topics: z.array(z.object({
    ...
    facts: z.array(z.object({
      ...
      correctionQuote: z.string().max(2400).nullable().describe("Exact source quote explicitly correcting/replacing a prior statement and quoting that old statement; null if not explicit"),
      ...
      verificationQuote: z.string().max(1600).nullable().describe("Exact source quote stating a performed verification, explicit date and environment/version/build scope; no planned, negated or future check; null when unverified. A report is never a host-verified live test"),
