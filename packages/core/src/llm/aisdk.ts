...
export class AiSdkBrainLlm implements BrainLlm, TurnPlanCompiler {
  ...
  async answer(
    ...
    const citationNote = [
      ...(tools.searchEntries ? ["TYPED RETRIEVAL: for dates, sources, content types or exhaustive memory audits, use search_entries. For recent voice notes, voicenotes or VNs, start with {contentType:voice_note,order:newest,limit:5}, then read the returned exact evidenceRefs with read_note. Use the requested count when given. Omit query for a category-only list: query matches literal transcript words, not categories or recency. Add date/source filters only when the user requests them; recent alone means newest order. Use catalog capturedAt for capture dates and ordering; raw Log headings record processing time. Set exhaustive=true for complete inventories; the host enumerates within its declared budget. Read exact evidenceRefs and follow passage cursors before synthesizing. Echo the actual scope; complete lexical enumeration is not complete semantic recall. Pinned evidence is primary. Never treat conversation-history results as durable memory. If coverage is partial, say so and retain continuation; never claim an exhaustive answer or absence."] : []),
    ...
    const config = {
      ...
      tools: {
        ...
        ...(tools.searchEntries ? {
          search_entries: tool({
            description: 'Search complete local immutable memory entries with typed date/source/content filters before limits. For recent voice notes/VNs use {"contentType":"voice_note","order":"newest","limit":5} and omit query; use a different count if requested. query is only literal subject/transcript words, never category words like "voice notes" or "recent". Do not invent date/source filters. Read the returned exact evidenceRefs and use their capturedAt source time, not the processing time in Log headings. For bounded exhaustive audits set exhaustive=true: the host enumerates up to 8 pages per ask (20 entries per page) before synthesis. Follow the returned cursor with identical filters/order. Read each exact evidenceRef via read_note, including its passage continuations, before claiming complete coverage. A budget stop is partial, never absence. Filters are lexical AND substrings in title/content; dates inclusive ISO instants (date-only is midnight UTC). Conversation history is separate; pinned contextRefs remain primary.',
            inputSchema: z.object({
              query: z.string().optional().describe("Literal subject/transcript terms (AND substrings). Omit for category-only lists such as recent voice notes; use contentType and order instead."), source: z.enum(["cli", "mcp", "whatsapp", "telegram", "web", "drive", "selftest"]).optional(),
