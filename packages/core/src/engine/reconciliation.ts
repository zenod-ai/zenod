...
export async function applyReconciliation(prepared: PreparedReconciliation, operations: ReconciliationOperation[]): Promise<ReconciliationResult> {
  ...
  for (const operation of operations.slice(0, 24)) {
    ...
    if (operation.kind === "link_source") {
      ...
      const proposal: FactProposal | null = factKey ? {key: factKey, statement, renderedStatement:statement, ...(operation.kind==="conflict" ? {reportedConflict:true} : {}), ...(legacy ? {legacySupersedes:legacy} : {}), effectiveDate: classifiedFact?.effectiveDate??null, effectiveDateQuote: classifiedFact?.effectiveDateQuote??null,
