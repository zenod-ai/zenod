...
it('retains clear existing topic decisions while ambiguous creation remains pending', async () => {
  ...
  const classify = vi.fn().mockResolvedValue({confidence: 0.95, summary: 'mixed', tags: [], pages: [existing, created], topics: [topic, {...topic, topic: 'new', summary: 'unknown', pages: [created]}]});
...
it('returns relevant text from the tail of an oversized section with exact source offsets', async () => {
  ...
  const packet = await branchContext(dir, await scanVault(dir), [{topic: 'research', query: 'Zebrafish', paths: ['Projects/A.md']}]);
