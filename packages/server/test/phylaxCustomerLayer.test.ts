...
import { createPhylaxCustomerLayer } from "../src/phylaxCustomerLayer.js";
...
function fixture() {
  const dataDir = mkdtempSync(join(tmpdir(), "phylax-customer-"));
  dirs.push(dataDir);
  let checkoutInput: Stripe.Checkout.SessionCreateParams | undefined;
  const identity: IdentityProvider = {
    authorizeUrl: () => "https://github.com/login/oauth/authorize",
    exchangeAndGetUser: async () => ({ id: 42, login: "phylax-owner" }),
  };
  const stripe = {
    checkout: {
      sessions: {
  ...
  const layer = createPhylaxCustomerLayer({ dataDir }, { env, identity, stripe });
...
describe("Phylax customer-layer duplicate", () => {
  ...
  it("returns GitHub sign-in to the Phylax landing", async () => {
    const { layer } = fixture();
    const state = signState({ mode: "signin", rh: "phylax.zenod.dev" }, env.ACCOUNT_STATE_SECRET!);
    const response = await layer.app.request(`/auth/github/callback?code=ok&state=${encodeURIComponent(state)}`);
    expect(response.status).toBe(302);
    expect(response.headers.get("location")).toBe("https://phylax.zenod.dev/");
  });

  it("binds checkout metadata and account storage to Phylax", async () => {
    ...
    layer.accounts.upsert("seed", {
      ...
      github_id: 42,
      github_login: "phylax-owner",
    });
    const stored = JSON.parse(readFileSync(join(dataDir, "customer-accounts-phylax.json"), "utf8"));
    expect(stored.seed.product).toBe("phylax");

    await createCustomerCheckout(
      stripe,
      layer.accounts,
      { domain: "https://phylax.zenod.dev", stripeMode: "test", stripeWebhookSecret: "", prices: {} },
      { github_id: 42, login: "phylax-owner" },
      { tier: "monthly", price: "price_monthly" },
      { product: "phylax", unit: "phylax", defaultDomain: "https://phylax.zenod.dev" },
    );
    expect(checkoutInput()?.metadata).toMatchObject({ product: "phylax", unit: "phylax" });
    ...
    expect(checkoutInput()?.success_url).toBe(
      "https://phylax.zenod.dev/checkout/complete?session_id={CHECKOUT_SESSION_ID}",
    );
  });

  it("turns a signed TEST webhook into a local Phylax tenant row", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "phylax-webhook-"));
    dirs.push(dataDir);
    ...
    const layer = createPhylaxCustomerLayer(
      { dataDir },
