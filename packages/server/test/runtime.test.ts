...
describe("runtime tasking tools", () => {
  ...
  it("AI tasking invalidates a revoked tenant installation and makes no further mutation attempt", async () => {
    const { privateKey } = generateKeyPairSync("rsa", { modulusLength: 2048 });
    runtime.settings.set("github_token", "");
    runtime.settings.setRaw("github_app_id", "tasking-app");
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (path === "/app/installations/111/access_tokens") {
        return Response.json({ token: "revoked-token", expires_at: new Date(Date.now() + 3600_000).toISOString() }, { status: 201 });
      }
      if (path === "/repos/zenod-ai/fixture/issues") return new Response("Bad credentials", { status: 401 });
    ...
    await expect(tools.createIssue({ title: "still blocked", body: "no mutation" })).rejects.toMatchObject({
      code: "github_connection_required",
    });
    expect(calls).toEqual([
      "/app/installations/111/access_tokens",
      "/repos/zenod-ai/fixture/issues",
    ]);
  });
