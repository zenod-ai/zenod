...
describe("GitHub compatibility fixtures", () => {
  it("preserves current store/search/get/ask, MCP, and peer receipt shapes", async () => {
    const fixture = JSON.parse(await readFile(
      ...
      get: Note;
      ask: Answer;
      mcpStructuredStore: StoreResult;
      peerReceipt: { status: string; kind: string; result: StoreResult };
    };
    ...
    expect(fixture.mcpStructuredStore).toMatchObject({
      evidenceRef: fixture.store.evidenceRef,
      pagesTouched: fixture.store.pagesTouched,
      commitSha: fixture.store.commitSha,
      filing: fixture.store.filing,
    });
    expect(fixture.peerReceipt).toMatchObject({
      status: "done",
      kind: "store",
      result: {
        commitSha: fixture.store.commitSha,
