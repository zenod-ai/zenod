...
import { hashToken, type UnitContext } from "@zenod/mcp-chassis";
...
import { PhylaxPortedRuntime } from "./phylaxPortedRuntime.js";
...
export function createPhylaxUnit(options: CreateZenodUnitOptions = {}) {
  ...
  let runtime!: PhylaxPortedRuntime;
  let tenantSettings!: PhylaxTenantSettingsStore;

  const base = createZenodUnit({
    ...options,
    agent: PHYLAX_AGENT,
    unitName: "phylax",
  ...
  tenantSettings = new PhylaxTenantSettingsStore(base.storage.dataDir, base.storage);
  const organ = new PhylaxChannelsOrgan({
    dataDir: base.storage.dataDir,
    ...
    transcriber: {
      async transcribe(input) {
        ...
        const result = await transcribeAudio(
          ...
          {
            // Empty strings deliberately suppress process-level fallback keys:
            // a hosted tenant can never inherit another/global STT credential.
            ...
            ...(transcription.provider === "openrouter"
              ? {
                  openrouterModel: transcription.model ?? undefined,
                  longTranscriptionProvider: "openrouter" as const,
                }
              : {}),
          },
        );
  ...
  runtime = new PhylaxPortedRuntime(base.storage.dataDir, organ, env, {
