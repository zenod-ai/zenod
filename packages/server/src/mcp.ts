import { mergeMemoryEntries, searchMemoryEntryPage } from "./memoryEntryCatalog.js";
...
export function buildMcpServer(
  ...
  const searchEntryPage = (engine: BrainEngine, input: EntrySearchInput): Promise<EntrySearchResult> =>
    searchMemoryEntryPage(engine, taskJobs, input);
  ...
  server.registerTool(
    "search_memory",
    {
      title: "Search memory",
      description:
        "Deterministic model-free memory search. For recent voice notes/voicenotes/VNs use contentType=voice_note, order=newest, limit=5 (or the requested count), and omit query. query is literal subject/transcript words, not category or recency words. Add date/source filters only when requested. Use entry capturedAt as the source capture time; raw Log headings can reflect processing time. hits are independent ranked lexical note paths, NOT constrained by entry filters. Structural filters/order or a cursor enable paginated entries: query terms are ANDed case-insensitive whitespace substrings in entry title/content, with all metadata filters applied before the page limit. Repeat query/filters/order with nextCursor until hasMore=false. A changed snapshot or process requires restarting and discarding prior pages. Coverage describes the complete local vault plus available retained receipts, not remote unsynced/deleted history. Pass exact evidenceRef to get_memory and read it before summarizing.",
  ...
  server.registerTool(
    ...
    async ({
      ...
      if (contentType && mediaType !== "audio") {
        return {
          content: [{ type: "text", text: "Audio contentType requires mediaType audio." }],
          structuredContent: { code: "invalid_input", message: "Audio contentType requires mediaType audio." },
          isError: true,
        };
      }
