...
| [#947](https://github.com/zenod-ai/zenod/issues/947) | Ticket worker | `wp2-live-sla` | W-P2 follow-up: bound and accelerate Phylax voice transcription | live verified | W-P2/W-P3 merged | [#959](https://github.com/zenod-ai/zenod/pull/959) / `9e429a2`; live `e50effd` | `5b7603a` | Preserve FIFO peak 1; bound transcription; typed failure handoff; truthful progress; exactly one terminal reply. | Provider `3B9250886A0B8323E441` transcribed by multilingual `base` in 10,294ms and completed in 15,756ms with one progress and one terminal outbound. | 2026-07-13 20:41 CEST | Done; retain regressions and live model prewarm. |
| [#948](https://github.com/zenod-ai/zenod/issues/948) | Ticket worker | `wp3-failure-audit` | W-P3 follow-up: redact Baileys session logs and persist timing audit | live verified | W-P3 merged | [#960](https://github.com/zenod-ai/zenod/pull/960) / `6c85929`; live `e50effd` | `9e429a2` | No session secrets in logs; safe health logs remain; additive provider-ID queue/transcription/downstream/outbound timings. | Provider-ID trace joins authenticated artifact, transcript/provenance, Ring correlation, receipt, two typed outbounds, and all six timings; libsignal detail remains redacted. | 2026-07-13 20:41 CEST | Done; retain provider-ID audit regression. |
| [#957](https://github.com/zenod-ai/zenod/issues/957) | Ticket worker | `wp6-credential-drift` | Detect and recover stale Ring downstream tenant credentials | live verified | Ring active tenant token | [#961](https://github.com/zenod-ai/zenod/pull/961) / live `e50effd` | `6c85929` | Detect rejected rotation without exposing tokens; recover inside Phylax encrypted custody; deterministic provider ID reaches the active tenant Ring MCP face. | Fresh provider reached `ring.zenod.dev#tenant:github-63050995`; receipt correlation `test_e50576693ee44660aca1625c1e056689` exists and credential status is `healthy`. | 2026-07-13 20:41 CEST | Done; monitor non-secret credential health. |
...
| W-C1a | Ticket worker | Calli agent | Expose first-class safe `draft_post` held-state tool | live-verified | W-C1 | [#949](https://github.com/zenod-ai/zenod/issues/949) / [#950](https://github.com/zenod-ai/zenod/pull/950) / code `32a7d58`; live `09fe2c4` | `a99db25` | Zero upstream call; typed tenant-held action; old `createPosts` flow remains compatible. | Exact live health SHA; authenticated 20-tool catalog; production held action remains pending with zero receipts; engine preserved. | 2026-07-13 03:24 CEST | Calli lane complete; Ring owns catalog refresh/selection and `/root` owns the later public WJ-2 journey. |
...
| 2026-07-13 | Phylax live voice acceptance | `080fe729ded8c64e850ba53e756465b677d62a63` | Real WhatsApp provider `3BA091E37C168738F529` + authoritative Phylax/Ring state | FAIL: custody/progress/terminal loudness pass; 12s PTT times out at 60s, Ring rejects stale credential, and failure path omits the typed audit row. | [#947 failure](https://github.com/zenod-ai/zenod/issues/947#issuecomment-4953770522), [#948 failure](https://github.com/zenod-ai/zenod/issues/948#issuecomment-4953770704), [#957](https://github.com/zenod-ai/zenod/issues/957). |
| 2026-07-13 | Phylax downstream credential recovery | live configuration on `080fe729` | Phylax encrypted tenant vault to active Ring tenant MCP face | PASS: URL and bearer synchronized without exposing tokens; authenticated `tools/list` connected; zero restart; `phylax-data` and 3,931 WhatsApp session files preserved. | [#957 recovery evidence](https://github.com/zenod-ai/zenod/issues/957#issuecomment-4954219449). |
| 2026-07-13 | Phylax live-failure repair candidate | `e50effd4d27716b7054ad6df86bf602980ff11ff` | merged `main` via #959/#960/#961 | PASS pre-deploy: base-model SLA fix, typed terminal-failure audit, and durable credential drift UX/custody passed focused suites, typechecks, web build, full CI, and independent reviews; #961 blockers fixed and re-reviewed. | [#959](https://github.com/zenod-ai/zenod/pull/959), [#960](https://github.com/zenod-ai/zenod/pull/960), [#961](https://github.com/zenod-ai/zenod/pull/961). |
| 2026-07-13 | Phylax live-failure repair deployment | `e50effd4d27716b7054ad6df86bf602980ff11ff` | Authoritative Swarm service + `phylax-data` | PASS deployment: exact immutable digest/health, zero restarts, preserved WhatsApp session, `base` prewarmed, authenticated Ring probe, encrypted URL/token migration, complete failure schema, both channel sockets, and zero unintended traffic/errors. | [#947 deploy](https://github.com/zenod-ai/zenod/issues/947#issuecomment-4960072831), [#948 deploy](https://github.com/zenod-ai/zenod/issues/948#issuecomment-4960073028), [#957 deploy](https://github.com/zenod-ai/zenod/issues/957#issuecomment-4960073204). |
| 2026-07-13 | Phylax final live PTT acceptance | `e50effd4d27716b7054ad6df86bf602980ff11ff` | Real WhatsApp provider `3B9250886A0B8323E441` + authoritative Phylax/Ring state | PASS: 14,674-byte artifact fetched and SHA-verified; multilingual `base` produced the `Cheesecake` transcript; one Ring handoff/receipt; one progress plus one terminal outbound; all six timings populated; total lifecycle 15,756ms; credential healthy; zero restart. | Ring correlation `test_e50576693ee44660aca1625c1e056689`; artifact SHA-256 `49b0d6df9d18f339a247a133132df6e2b1e7ba68695237fcc46ae389488c550c`; issues #947/#948/#957. |
...
### 2026-07-13 - phylax-agent - live voice acceptance failed with credential drift

Evidence: Real provider `3BA091E37C168738F529` was received at 01:26:46.882Z, archived as a 25,072-byte OGG with SHA-256 `3899f29621845a9313f3badaf82d1724435de6e397274587562161ce43203119`, and received the owner-only progress outbound `3EB0ED39827F49821878BA` at +6.482s. The 12-second note hit the 60-second transcription deadline; no whisper/ffmpeg process remained afterward. The terminal outbound `3EB0374E0A50C1C2072007` arrived at +60.730s.

Cause: Ring's active tenant credential rotated at 01:22:34.403Z, 4m12s before the note. Phylax's encrypted downstream bearer still matched its configured URL path but not Ring's new active tenant hash, so the typed timeout handoff received `{"error":"unauthorized"}`. Neither credential was logged; diagnosis compared only SHA-256 fingerprints and tenant hashes. Ring routing and service state were not changed.

Audit gap: the provider exists in raw message custody, outbound audit, and media coalescing, but not `whatsapp_channel_audit`. The failure path updates timing after no success row was inserted, so its six typed timings and downstream failure correlation are absent. #947 and #948 are reopened; #957 owns stale downstream credential recovery.

Gate at failure: production credential write required Jordi/root authorization before replacing only Phylax's encrypted downstream URL/bearer. Completion still requires transcript/artifact, populated failure-or-success timings, Ring correlation, exactly one terminal provider ID, and screenshot.

Recovery update: Jordi authorized the credential write. Phylax's encrypted downstream path and bearer now match Ring's active tenant token; an authenticated `tools/list` probe passed. Phylax did not restart, and its QR/session/media state was untouched. #957 remains open only for durable rotation detection/recovery; #947 and #948 still require code fixes before the next real PTT acceptance walk.

Merged update: #959, #960, and corrected/re-reviewed #961 are now on `main`; exact deployment candidate is `e50effd4d27716b7054ad6df86bf602980ff11ff`. Phylax defaults/prewarms multilingual `base`, persists typed secret-free terminal-failure traces, tracks credential health against a non-secret revision, ignores stale in-flight status after replacement, and stores credential-bearing Ring MCP URLs in encrypted custody with restart-idempotent legacy scrubbing. No Ring semantics changed.

Deployment update: Jordi authorized and the authoritative Swarm service is live on immutable `sha-e50effd@sha256:0cd9f6f35138f7db812a703a3304ddbd92bc013ab562d3f0a734b0b70d846bfd`. Public health reports exact SHA; task restart count is zero; `phylax-data`, WhatsApp credentials, and 3,931 session files are preserved. `ggml-base.bin` is ready, the Ring `tools/list` probe connects, plaintext downstream URL is scrubbed, encrypted URL/token and credential revision are present, all failure/timing columns exist, WhatsApp/Telegram TLS are established, and rollout created no messages.

Assignment identity: `phylax-agent`
Branch / deployed code: `codex/phylax-live-failure-spine` / `080fe729ded8c64e850ba53e756465b677d62a63`
Base / integration target: `5b7603a` / `main`
Last verified: 2026-07-13 03:55 CEST
Links: [#947](https://github.com/zenod-ai/zenod/issues/947), [#948](https://github.com/zenod-ai/zenod/issues/948), [#957](https://github.com/zenod-ai/zenod/issues/957).

### 2026-07-13 - phylax-agent - final live voice journey passed

Outcome: The fresh WhatsApp PTT `3B9250886A0B8323E441` passed on exact live SHA `e50effd4d27716b7054ad6df86bf602980ff11ff`. Phylax archived and authenticated the 14,674-byte voice artifact, verified SHA-256 `49b0d6df9d18f339a247a133132df6e2b1e7ba68695237fcc46ae389488c550c`, and transcribed the spoken deterministic code word `Cheesecake` with multilingual `whisper.cpp base`.

Typed trace: media download 104ms; queue wait 1ms; transcription 10,294ms; Ring downstream 5,125ms; outbound send 95ms; total lifecycle 15,756ms. Ring tenant `github-63050995` recorded correlation `test_e50576693ee44660aca1625c1e056689` with an `ok` receipt. WhatsApp recorded one owner-only processing outbound `3EB0EAAEC8A75A4F8327EF` and one terminal sent outbound `3EB0B5F7F159333B360900`; the inbound lifecycle is `replied` with no failure.

Safety: credential health is `healthy`; the plaintext downstream URL remains absent; the encrypted URL/token revision remains present; the authoritative task is still on the exact SHA with zero restarts. No pairing reset, provider-session deletion, Ring routing change, or unrelated unit change occurred.

Disposition: #947, #948, and #957 have live acceptance evidence and may close. The Phylax lane is complete; remaining whole-batch customer journeys belong to their owning lanes and `/root`.

Assignment identity: `phylax-agent`
Live commit: `e50effd4d27716b7054ad6df86bf602980ff11ff`
Last verified: 2026-07-13 20:41 CEST

## Open Questions

- None for the Phylax lane. Final live PTT acceptance passed without a pairing reset.
