...
Context: H-S7's natural-language router now sends affirmative show/propose, numbered approval, feedback, and publish requests into the real Herald lanes. Feedback is filed to Zenod and locally before the next wake; model turns expose read-only wallet tools only; publish/propose classifiers include negation and retrospective no-mutation guards. PR #911 passed 720/720 server tests, typecheck/build, Docker build check and CI, then merged as exact integrated R2 commit `6ce6e95fc97795d5ceca3df8ccd15deb1527a0ac`.

Assignment: H-S9-worker / `codex/h-s9-deterministic-seams` / `../wt-h-s9`, pinned to `6ce6e95fc97795d5ceca3df8ccd15deb1527a0ac`. H-S9 remains hard-capped at 90 minutes and may add only deterministic seam coverage; conversational quality stays with H-S5.

Next: integrate green H-S9 tests, run preflight, deploy one exact SHA, and restart H-S5 from step 1. Total H-D17 stop remains 18:08 CEST.
...
Context: fresh `origin/main` was pinned at `b86a28b33e4783c7441ddfa00f9dccb4c1b17c77`. The accepted backlog is materialized as H-S6 [#905](https://github.com/zenod-ai/zenod/issues/905), H-S8 [#906](https://github.com/zenod-ai/zenod/issues/906), H-S9 [#907](https://github.com/zenod-ai/zenod/issues/907), and H-S7 [#908](https://github.com/zenod-ai/zenod/issues/908). The four-hour H-D17 clock began with R1 dispatch and expires at 2026-07-12T18:08:00+02:00.

Assignments: H-S6-worker / `codex/h-s6-state-kernel` / `../wt-h-s6`; H-S8-worker / `codex/h-s8-herald-language` / `../wt-h-s8`; both must create their dedicated worktree as first action from the exact pinned base. H-S7 and H-S9 remain dependency-gated.

Next: integrate green R1 work, pin the integrated SHA for H-S7, then execute H-S7 → deterministic H-S9 + preflight within the total cap.
