...
Last reconciled commit: `6f16985`
...
- [ ] DX-1 inventory: the original authoritative inventory completed, but a newly minted record-only duplicate made it stale; #793 is refreshing every remaining row and classification.
...
Phase: DX-2 complete; upstream Zenod pilot rejected; classification integrated; cloud recovery remediation active
Last verified: 2026-07-10 05:54 CEST
Integration target: main
Fresh base commit: `6f1698578e5f46466507c9520f88579059b54c94`
Next action: merge the reviewed #793 integration, then integrate cloud #62's duplicate-safe recovery fix while Epic 3.1 resolves encrypted-vault blocker #789. After #789, Epic 3.2 must rebase #791 to exact main, publish an immutable image, and rerun fresh full mode before #738 may begin P0. Keep cloud `tenants_api` off.
Blockers: independent acceptance rejected #736/#738 at product `2d8509e`: the retained run was contract-only, all three vault clones failed, Repo/Ingest/Usage and self-host commit parity were absent, and full migration/idempotency/rollback/restore receipts were missing. #789 must encrypt chassis vault DB/WAL/SHM bytes. The full rerun then needs a capped test inference key and approved marker commits to three existing disposable repos. Epic 3.3 and 3.5 remain draft; Alpha9 has no canonical multi-tenant Callisthenes/Ring runtime; endpoints remain 401/404/404. A new Callisthenes duplicate record also makes DX-1 stale until #793/cloud #62 land. Live waves still require exact digest/window/archive/rollback approval. DX-2 itself remains complete.
...
| [#793](https://github.com/zenod-ai/zenod/issues/793) | Ticket worker | Zeno (`019f4a18-5a81-74e3-a238-32b286a81e24`) | DX-1B classify retained test rows and refresh post-probe inventory | review / integrated local | #714; cloud #62 informs cause | worker `78c786c` + `ebfb51e`; integration `9f4cd53` on `codex/epic37-classification-recovery-reconcile` | Fresh exact inventory; Callisthenes/Ring test classifications; suite-bundled Epaminon topology; fail-closed duplicate packet; no apply. | Fresh current-state/checksum and exact duplicate candidate/checksum are integrated. Independent combined tests pass; stale causal statement was caught and corrected to the proven live `status-reconcile` path. | 2026-07-10 05:54 CEST | Push integration PR, pass CI, merge, then close issue as package-complete; cleanup remains a separate human gate. |
...
| [#728](https://github.com/zenod-ai/zenod/issues/728) | Ticket worker | Hooke (`019f49f1-f978-7110-9dbf-f1b1a5ffd7a4`) | DX-3 Zenod retirement wave | blocked / package merged | #789 + accepted #736 full proof + #738 Gate 2 + #714 | [PR #787](https://github.com/zenod-ai/zenod/pull/787) merged as `7403d0c`; prep `codex/epic37-dx3-zenod-wave` | `1d7f440` | All per-user Zenods retired with per-tenant verification. | Independent audit rejected #736/#738 at branch `3688410` / product `2d8509e`: automated suites pass, but custody, full commit/marker, browser Repo/Ingest/Usage, self-host parity, and migration/restore acceptance do not. Exact one-row package still has no apply mode. | 2026-07-10 05:38 CEST | Merge #789, rerun accepted full-mode #736 on exact main/immutable image, complete #738 migration and observation gates, then obtain Jordi's separate digest/window/archive/rollback approval. |
| [#729](https://github.com/zenod-ai/zenod/issues/729) | Ticket worker | Pasteur (`019f49f1-f9ef-7ed3-b6ba-0fc552f151f7`) | DX-4 Callisthenes retirement wave | blocked / classification refresh | parent pilot + 3.3 CA-MT-6 + #793 | [PR #787](https://github.com/zenod-ai/zenod/pull/787) merged as `7403d0c`; #793 `codex/epic37-dx1b-classification` | `1d7f440`; refresh `6f16985` | All per-user Callisthenes retired with per-tenant verification. | Legacy row is now evidence-backed `test`, bound to `jordi+calli-fresh-33087769@alpha9.io` and a Stripe TEST session; shared `tenant_id` remains null. A new record-only duplicate exists. There is no apply mode. | 2026-07-10 05:38 CEST | Integrate #793, then consume pilot/CA-MT-6, shared tenant binding, credential/receipt isolation, old-token continuity, rollback, archive, and Jordi approval. |
| [#730](https://github.com/zenod-ai/zenod/issues/730) | Ticket worker | Meitner (`019f49f1-fa41-7660-aa14-148e3453a34c`) | DX-5 Epaminon retirement wave | blocked / classification refresh | parent pilot + spawner decision + 3.5 E-MT-7 + #793 | [PR #787](https://github.com/zenod-ai/zenod/pull/787) merged as `7403d0c`; #793 `codex/epic37-dx1b-classification` | `1d7f440`; refresh `6f16985` | All per-user Epaminons retired; AWAIT_PROVISION fleet gone. | Ring row is evidence-backed `test`; its Epaminon container/volume are `suite-bundled`, not an independent tenant. Canonical shared `epaminon` remains out of scope. | 2026-07-10 05:38 CEST | Integrate #793; then obtain pilot/spawner/E-MT-7/sandbox proof, Ring job-history/archive mapping, fresh inventory, snapshot/rollback, and exact approved removal scope. |
...
| [cloud #62](https://github.com/zenod-ai/cloud/issues/62) | Ticket worker | Huygens (`019f4a18-615c-7471-bfcb-346c1551e1e5`) | Make legacy provision recovery duplicate-safe | running | observed fleet drift; informs #731/#793 | `codex/issue62-recovery-idempotency`; `/tmp/cloud-issue62-recovery` | Side-effect-free status; environment mismatch 409; single-flight recovery; pre-create adopt/fail closed; exact-resource success proof; production mode unchanged. | Live logs conclusively show deployed `status-reconcile` GET replayed a cloud-test checkout on live due duplicate hostname ownership, created the duplicate, then falsely accepted the original hostname. Worker redirected to exact seven-point regression. No cleanup authorized. | 2026-07-10 05:40 CEST | Implement deterministic regression and fix, test, commit, and hand off; separately correct deployment hostname ownership under an explicit deployment gate. |
...
| Epic 3.2 full-mode pilot resources | Jordi | After #789 merges and #791 rebases | Approve one capped test-only OpenRouter key and marker commits to the three named disposable repos | Contract-mode tests and non-mutating preparation |
...
- Zeno (`019f4a18-5a81-74e3-a238-32b286a81e24`) completed #793 worker commits `78c786c` + `ebfb51e`; steward integration `9f4cd53` passed independently.
- Huygens (`019f4a18-615c-7471-bfcb-346c1551e1e5`) owns cloud #62 in `/tmp/cloud-issue62-recovery` from cloud main `3e80e7b`.
...
- DX-1's one live-paying row remains `zenod-jordi-f2c7a6`. The retained Callisthenes and Ring rows are now evidence-backed Stripe TEST, with Ring's Epaminon executor classified `suite-bundled`. New record-only duplicate `Us9aDVdhvlObXLDfDwW0I` is unapproved and makes the old inventory stale. #793 owns the refresh and exact cleanup packet; cloud #62 owns duplicate-safe recovery. Owner: Epic 3.7 steward/workers. Needed by: next test-row approval and later waves.
- After #789 merges and #791 rebases, Jordi must narrowly approve minting one capped test-only OpenRouter inference key and writing proof commits to `AlfaBlok/test_evals`, `AlfaBlok/react_test1`, and `AlfaBlok/zenod-cloud-test-vault-4ptjqj`. This is not production migration or retirement approval. Owner: Jordi. Needed by: #736 fresh full-mode acceptance.
