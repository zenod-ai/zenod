...
Phase: ZAL-7 and ZAL-8 integrated; ZAL-9 running; ZAL-10 held in a bounded review-fix loop
Last verified: 2026-08-25 CEST
Integration target: `main`
Fresh base commit: `dd41a9c` on `main`
Next action: reconcile ZAL-9's reuse-first activation handoff and ZAL-10's corrected admission/read-boundary SHA; integrate each only after independent review and version-coherent evidence.
...
| [#1075](https://github.com/zenod-ai/zenod/issues/1075) | Ticket worker | ZAL-9-whatsapp-activation-worker | ZAL-9 · Reuse existing channel components for Hosted WhatsApp activation | running | #1073, #1074 | `codex/zal-9-whatsapp-activation` | `dd41a9c` | Existing WhatsApp verification/status/test components operate tenant-scoped inside Zenod; self-host remains Telegram-only. | Dedicated worktree/branch dispatched from exact validated `main`; reuse-only scope and no-live-session gate recorded in issue #1075. | 2026-08-25 CEST | Worker implements/tests and posts structured handoff; steward independently reviews before integration. |
| [#1076](https://github.com/zenod-ai/zenod/issues/1076) | Ticket worker | ZAL-10-managed-ai-worker | ZAL-10 · Extend existing billing and metering for managed Hosted usage | running / review fixes | approved economics | [PR #1086](https://github.com/zenod-ai/zenod/pull/1086) / `codex/zal-10-managed-ai` | `fa41246` | Existing customer/billing/metering stores gain managed cap lifecycle; Hosted is percentage/state/reset only; self-host/raw operator evidence remains. | Corrected `17b2cce` closed the first seven blockers and passed CI/all-workspace tests, but re-review found Hosted settings metadata disclosure, Telegram admission bypass/raw-loss, and a downstream-commit-before-admission-receipt replay window. | 2026-08-25 CEST | Same worker closes the three exact blockers with cookie/bearer, Telegram media/restart, and post-commit crash/idempotency regressions; steward re-reviews; no live key/billing mutation. |
