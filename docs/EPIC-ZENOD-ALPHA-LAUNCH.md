...
Phase: ZAL-7, ZAL-8, and ZAL-10 integrated; corrected ZAL-9 reconciling the combined channel/policy seams
Last verified: 2026-08-26 CEST
Integration target: `main`
Fresh base commit: `ab7b50e` on `main`
Next action: independently review ZAL-9's corrected current-main head, then run the combined Hosted channel/policy/admission gate on the exact merge result before integration.
...
| [#1075](https://github.com/zenod-ai/zenod/issues/1075) | Ticket worker | ZAL-9-whatsapp-activation-worker | ZAL-9 · Reuse existing channel components for Hosted WhatsApp activation | running / review fixes | #1073, #1074 | [PR #1087](https://github.com/zenod-ai/zenod/pull/1087) / `codex/zal-9-whatsapp-activation` | `dd41a9c` | Existing WhatsApp verification/status/test components operate tenant-scoped inside Zenod; self-host remains Telegram-only. | Initial `519a1f4` passed CI and broad tests, but review blocked private-response leakage, non-idempotent mutations, reload/degraded UI traps, missing Hosted Telegram lifecycle, self-host route exposure, and incomplete tenant/config boundaries. | 2026-08-25 CEST | Same worker closes exact blockers and reconciles final ZAL-10 seams; steward re-reviews; no live session/route mutation. |
