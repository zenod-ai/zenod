...
Phase: Minimum implementation and ZAL-17 local release-candidate proof integrated; ZAL-4 read-only production preflight is dispatchable
Last verified: 2026-08-26 CEST
Integration target: `main`
Fresh base commit: `91b4e7d` on `main`
Next action: prepare the read-only [ZAL-4 #1061](https://github.com/zenod-ai/zenod/issues/1061) preflight packet with exact immutable image digest, Dokploy target, redacted environment-key delta, backups, rollback, and staging checks; stop for Jordi's explicit deployment/configuration approval.
...
| [#1061](https://github.com/zenod-ai/zenod/issues/1061) | Epic worker / operator | unassigned | ZAL-4 · Execute the fail-closed production-readiness gate | ready / approval-gated | [#1083](https://github.com/zenod-ai/zenod/issues/1083), production approval | `codex/zal-4-production-gate` | `91b4e7d` | Every runbook check has current evidence; signup remains closed until exact approval, then opens and verifies or rolls back safely. | ZAL-17 local candidate and durable evidence are integrated; no live/staging proof yet. `docs/PRODUCTION-READINESS.md` remains fail-closed. | 2026-08-26 CEST | Prepare read-only packet only; request exact image/target/env/rollback approval before any deploy/config. Billing and signup remain separate gates. |
...
Assignment identity: Zenod Alpha delivery manager; ZAL-4 production-readiness operator (next read-only dispatch)

Branch / latest commit: `main` at `91b4e7d`; no ZAL-4 branch yet
