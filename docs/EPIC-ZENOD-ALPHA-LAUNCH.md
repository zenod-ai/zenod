...
Phase: Minimum implementation and single €9 contract integrated; automatic immutable image and ZAL-4 read-only refresh next
Last verified: 2026-08-26 CEST
Integration target: `main`
Fresh base commit: `ba5e9c10ff6466e357568fe9d495913af8047631` on `main`
Next action: wait for the repository's existing automatic `Publish image` workflow for `ba5e9c1`, then refresh [ZAL-4 #1061](https://github.com/zenod-ai/zenod/issues/1061) read-only with the exact OCI/amd64 digest and corrected site/account/Terms/readiness truth; stop before any external production mutation.
Blockers: exact off-host backup destination/procedure, backup quiesce, credential/token sources and reviewed non-secret config values, live €9 Stripe price selection, and immutable-image/Dokploy/rollback approval are unresolved. OpenRouter reconciliation, Google OAuth, live channel tests, real-card billing, signup, and promotion remain later independent gates.
...
| [#1061](https://github.com/zenod-ai/zenod/issues/1061) | Epic worker / operator | ZAL-4-production-readiness-operator | ZAL-4 · Execute the fail-closed production-readiness gate | running / read-only refresh | automatic immutable image for `ba5e9c1`, then named production approvals | merged docs [PR #1089](https://github.com/zenod-ai/zenod/pull/1089); refresh branch after digest | `ba5e9c1` | Every runbook check has current evidence; signup remains closed until exact approval, then opens and verifies or rolls back safely. | Prior packet is exact for live state; ZAL-18 removed its pricing blocker and merged as `ba5e9c1`; no external production mutation. | 2026-08-26 CEST | Record the automatic digest and refresh packet truth only; stop at unresolved backup/credential/config/deploy/provider/channel/billing/signup gates. |
...
- Active: #1061 read-only packet refresh after the normal main image publication for exact `ba5e9c1`.
