...
Phase: fully specified 22-screen Zenod product/backend contract at final Jordi sign-off
Last verified: 2026-08-25 CEST
Integration target: `main`
Fresh base commit: `3c0d084` on `main`
Next action: Jordi reviews `docs/evidence/zenod-whatsapp-public-beta-2026-08-25/ui-contract.html` and approves or revises the binding 22-screen beta contract: seven Hosted customer screens, seven supported self-host screens, and eight protected owner-admin screens, each with concrete controls, reads, mutations, states, permissions, and backend requirements. After sign-off, the delivery manager creates the smallest dependency-ordered implementation batch from that contract.
Blockers: only final product/UI contract sign-off remains. Hosted deploys public Zenod plus the existing private Phylax service for WhatsApp. The supported self-host package starts one Zenod container with Telegram and does not call or start Phylax; Phylax source and functionality remain intact for Hosted or advanced manual operation. Raw Operating Rules/SEAM/Skills diagnostics are absent from the beta navigation until they have a clear customer action. Production mutation, live billing drill, signup opening, WhatsApp-session change, and external promotion remain separate later approvals.
...
| 9 | `docs/evidence/zenod-whatsapp-public-beta-2026-08-25/index.html` and `ui-contract.html` beside it | Current 48-hour WhatsApp audit, deployed topology, Zenod-only packaging recommendation, binding 22-screen UI/backend contract, final code iteration, and production plan. | Planner / ZAL-3 / ZAL-4 |
...
| 2026-08-25 | Treat the 22-screen UI contract as the binding implementation surface. | Each visible screen and dialog now defines reads, mutations, states, permissions, safety behavior, and existing/required backend work. No navigation item is added without this contract. | `docs/evidence/zenod-whatsapp-public-beta-2026-08-25/ui-contract.html` |
...
| [#1060](https://github.com/zenod-ai/zenod/issues/1060) | Planner | ZAL-3-offer-decision-planner | ZAL-3 · Frame the alpha offer and WhatsApp boundary for decision | final 22-screen contract sign-off | Jordi approves or revises the complete €9 Zenod beta product and binding UI/backend contract | [merged PR #1068](https://github.com/zenod-ai/zenod/pull/1068) plus [ZAL-3E PR #1071](https://github.com/zenod-ai/zenod/pull/1071) / `main` | `0bb5b3d` | Truthful offer, usage economics, Zenod-only WhatsApp boundary, customer journey, editions, operator surface, exact screens/actions/states/backend, and exact Jordi decision are recorded. | The durable contract specifies seven Hosted, seven self-host, and eight owner-admin screens; every control is mapped, self-host runs one Zenod container without Phylax, and raw Rules/Skills diagnostics are excluded from beta navigation. | 2026-08-25 CEST | Jordi signs off `ui-contract.html` or names one remaining screen/action/product assumption to revise. |
...
|---|---|---|---|---|
| Final Zenod beta product | Jordi | Product boundary, editions, exact 22-screen UI/backend contract, operator surface, deployment, and economics are explicit | Reply `APPROVE FINAL ZENOD BETA PRODUCT` to approve one €9/month plus VAT hosted plan with managed AI usage and WhatsApp included; customer usage shown only as percentage/state/reset; hidden adjustable operator caps with no overage and queued raw evidence at the limit; existing Phylax retained privately as Hosted's second service; Ring absent; MCP first-class; supported free self-host starts one Zenod container with self-managed provider controls and Telegram but no Phylax/managed WhatsApp UI; the seven Hosted, seven self-host, and eight owner-admin screen contracts; no raw Rules/Skills beta nav and no alpha annual plan. Or name one assumption to revise. | Read-only checks, local validation, and draft-only prep. |
...
| 2026-08-25 | Zenod WhatsApp audit, public-beta proposal, and binding 22-screen UI/backend contract | working tree from `3c0d084` | Production Phylax/Ring/Zenod read-only stores and service metadata; repository customer/admin/edition code; OpenRouter primary pricing; standalone interactive HTML at 360/736/1024px | Reconciled 10 interactions; live bindings; current three/proposed Hosted two-service topology; supported one-container self-host shape; seven Hosted, seven self-host, and eight admin screens with complete controls/states/backend contracts | final product/UI packet ready for Jordi sign-off; no production, billing, session, signup, credential, user, or route mutation | `docs/evidence/zenod-whatsapp-public-beta-2026-08-25/index.html`; `docs/evidence/zenod-whatsapp-public-beta-2026-08-25/ui-contract.html` |
...
### 2026-08-25 - Epic worker - Binding 22-screen UI/backend contract ready

Context: Jordi required the mockup to become the implementation contract: every visible section must be a real screen with complete controls, states, permissions, and backend consequences. The new durable contract specifies seven Hosted customer screens, seven supported self-host operator screens, and eight protected Zenod owner-admin screens. It includes concrete dialogs for owner credential setup, tenant provisioning/suspension/removal, usage adjustment, MCP rotation, WhatsApp verification and shared-session operations, Drive setup, job inspection/retry, and Stripe configuration. Hosted uses the existing public Zenod plus private Phylax services. The supported self-host package starts one Zenod container, exposes Telegram and customer-owned model settings, and neither starts nor calls Phylax; the Phylax source/runtime remains intact for Hosted or optional advanced manual operation. Raw Operating Rules/SEAM/Skills diagnostic pages are removed from the beta navigation because they do not yet present a stable customer job.

Next: Jordi reviews `docs/evidence/zenod-whatsapp-public-beta-2026-08-25/ui-contract.html` and either replies `APPROVE FINAL ZENOD BETA PRODUCT` or names a specific screen/action/assumption to revise. After approval, the delivery manager converts the contract into the smallest dependency-ordered implementation batch. Production, credential, tenant, billing, session, signup, route, and publication mutations remain closed.
...
Branch / latest commit: `main` at `3c0d084`; UI contract, index revision, and spine reconciliation in current working tree
