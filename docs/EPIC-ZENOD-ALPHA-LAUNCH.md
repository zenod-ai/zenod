...
Last reconciled commit: `ba5e9c1` plus current preflight-refresh working tree
Planner: Jordi + Zenod Alpha delivery manager
Worker: ZAL-4-production-readiness-operator
Tester: unassigned
...
Fresh base commit: `fb184d0713db00dd72d9e5191b345eadcb0e8448` on `main`
Next action: review and integrate [ZAL-20 #1094](https://github.com/zenod-ai/zenod/issues/1094) plus [ZAL-21 #1095](https://github.com/zenod-ai/zenod/issues/1095), then publish the combined merge as the one final immutable image. Refresh [ZAL-4 #1061](https://github.com/zenod-ai/zenod/issues/1061), perform verified encrypted backups, signup-closed deployment and live acceptance until the candidate is ready for Jordi's final launch test.
Blockers: GitHub Actions reports `major_outage`; no digest may be inferred. The live €9 price, dedicated Channels secret and encrypted off-host backup remote are prepared. Remaining work is a Zenod-owned Google OAuth client/consent, Stripe profile correction, reviewed non-secret Dokploy delta, fresh backups, closed deployment and live acceptance. Public signup and promotion stay closed until the final test and explicit opening decision.
...
| [#1061](https://github.com/zenod-ai/zenod/issues/1061) | Epic worker / operator | ZAL-4-production-readiness-operator | ZAL-4 · Execute the fail-closed production-readiness gate | blocked / external runner outage | successful exact immutable image for `ba5e9c1`, then named production approvals | merged docs [PR #1089](https://github.com/zenod-ai/zenod/pull/1089); clean `codex/zal-4-preflight-refresh`; [run 32983888246](https://github.com/zenod-ai/zenod/actions/runs/32983888246) | `046ff07` control / `ba5e9c1` runtime | Every runbook check has current evidence; signup remains closed until exact approval, then opens and verifies or rolls back safely. | Pricing blocker is closed, but original publish had no runner/steps and the approved retry is queued during a GitHub Actions major outage; GHCR tag absent; live state unchanged. | 2026-08-26 CEST | After Actions recovery, consume the existing retry; on success record the real digest and refresh packet truth only. |
...
| [#1094](https://github.com/zenod-ai/zenod/issues/1094) | Ticket worker | ZAL-20-drive-least-privilege-worker | ZAL-20 · Make Hosted Drive least-privilege and launchable | running | [#1083](https://github.com/zenod-ai/zenod/issues/1083) | `codex/zal-20-drive-least-privilege` | `305dce7` | Hosted uses non-sensitive per-file Drive scope and one app-owned folder; existing self-host BYO behavior remains; no restricted full-Drive scope or customer credential UI. | Official Google guidance identifies current full Drive scope as restricted and security-assessment-gated; dedicated worker dispatched with no live Google mutation. | 2026-08-26 CEST | Deliver focused OAuth/folder/UI regressions and independent review before final image publication. |
| [#1095](https://github.com/zenod-ai/zenod/issues/1095) | Ticket worker | ZAL-21-backup-verifier-worker | ZAL-21 · Make two-volume backup verifier image-safe | running | [#1061](https://github.com/zenod-ai/zenod/issues/1061) | `codex/zal-21-backup-verifier` | `305dce7` | Existing guarded backup supports a pinned verifier image and mode-0600 artifacts without weakening pause/resume/refusal/cleanup behavior. | Read-only audit proved private current image lacks the verifier while the pinned public image contains it; focused worker dispatched; no live pause/backup. | 2026-08-26 CEST | Deliver shell regressions and review before the backup gate executes. |
