...
Last reconciled commit: `fd76aefca6030c2e55e115b9c797864b126b5aa5`
...
Phase: shipped; Ring exact-tool hardening deployed — generic MCP, truthful receipts/holds, and fail-closed cited reads live
Last verified: 2026-07-13T02:50:00+02:00
Integration target: main
Live Ring artifact: `ghcr.io/zenod-ai/zenod:sha-fd76aef` (`fd76aefca6030c2e55e115b9c797864b126b5aa5`)
Validated Zenod skill publisher: `ghcr.io/zenod-ai/zenod:sha-e7dc215` (`e7dc215a566189c317a68533a7006c6d8a5b2d8f`)
Next action: observe the channel-independent standing-approval contract when the channel owner supplies a real ingress turn. Keep concise-read presentation fail closed when the model cites a source that the same-turn MCP result did not return. Do not operate or modify Phylax, Zenod, or Calli from this lane.
...
Reconciled 2026-07-13 against `main` and live `ring.zenod.dev` at exact SHA `fd76aefca6030c2e55e115b9c797864b126b5aa5`.

- **Exact connected-tool identifiers (`fd76aef`):** Ring briefs the model that every discovered collision suffix is part of the callable identifier. If a provider omits only one final 16-hex suffix, AI SDK's pre-execution repair boundary may select the one exact connected candidate. The repair fails closed for ambiguity, partial bases, invented names, legacy/built-in tools, or invalid arguments. The exact tool's schema and mutation guard still run; the repair grants no new capability or authority.
- **Live exact-name proof (`fd76aef`):** the signed-in web chat repeated the read-only Ring-memory request that previously displayed a shortened phantom call. The new turn rendered only `zenod__search_memory__04c2430fa6f7552c`; runtime logs record one start and one end for that exact tool and no short-name error or duplicate upstream execution.
- **Fail-closed presentation proof (`fd76aef`):** the provider stopped after search and proposed a concise claim with a source URL not present in the same-turn returned evidence. `reply-gate` discarded that unsupported prose and rendered the bounded MCP result plus safe returned links. This is a truthful but rough fallback, not license to weaken citations or add a peer-specific shortcut; future presentation work must obtain or cite evidence the peer actually returned.
...
- **Closed Ring residual:** shortened collision-safe peer names are now repaired only when they identify one exact connected tool. The live turn showed no phantom short-name activity and one exact peer call. No peer alias, provider-specific retry, Zenod tool name, or channel phrase is hard-coded.
...
| 2026-07-13 | Exact-name repair boundary | Ring may repair only a uniquely omitted final 16-hex collision suffix for a currently connected MCP tool, before execution. Ambiguous, partial, invented, built-in, legacy, or argument-invalid calls fail closed; exact schema and mutation guards remain authoritative. |
...
| [#935](https://github.com/zenod-ai/zenod/issues/935) | Ticket worker | Ring agent / W-R3 | One logical MCP call and rendered outcome | done | - | [#943](https://github.com/zenod-ai/zenod/pull/943) / `codex/w-r3-logical-call-dedupe`; follow-up `fd76aef` | `c5768d3` | one upstream logical call and one rendered result per operation | live exact tool only, one upstream call; unique suffix repair is pre-execution and preserves guards | 2026-07-13T02:50:00+02:00 | closed |
...
| 2026-07-13 | Exact connected-tool repair | `fd76aef` | full repository + ring.zenod.dev signed-in web chat and runtime logs | 49 focused core tests; full `npm test && npm run typecheck`; repeat read-only Ring-memory prompt; exact-SHA routes/auth | PASS: unique omitted suffix repairs before execution; ambiguity and invalid args fail closed; mutation guard preserved; live turn rendered only exact `zenod__search_memory__04c2430fa6f7552c` and ran it once. Unsupported model citation was discarded in favor of returned MCP evidence. | CI + Publish image passed; live health `fd76aef`; root/app 200; MCP unauth 401; OAuth callback exact; Ring service kept one `/data` mount |

## Handoff Journal

### 2026-07-13T02:50:00+02:00 - Ring delivery manager - Generic exact-tool hardening live

Context: `fd76aef` adds a provider-independent, pre-execution repair for one narrowly defined model error: omitting the final 16-hex collision suffix from exactly one currently connected MCP tool. Focused tests prove unique repair, ambiguity/invention rejection, no argument repair, and preservation of the exact mutation guard. The full repository test and typecheck suites, GitHub CI, image smoke, and immutable image publication passed. The Ring Dokploy record accepted the exact image and environment update but its deploy/redeploy endpoints created no queue row; after Ring-scoped queue cleanup also produced no row, the manager reconciled only `ring-ycxjwn` to the already-recorded immutable image and `GIT_SHA`, preserving the single `/data` mount. No other service changed.
Reasoning: collision-safe names remain the authenticated catalog contract. Ring may correct one deterministic formatting omission only when there is exactly one possible connected target; it never guesses between capabilities, repairs inputs, maps friendly aliases, or bypasses schema/approval/mutation guards. This stays generic across peers and providers.
Live proof: the signed-in browser repeated the prior read-only memory prompt. The current turn displayed only `zenod__search_memory__04c2430fa6f7552c`, and runtime logs record exactly one start/end pair. The provider then cited a URL outside the returned evidence; Ring correctly discarded that prose and rendered bounded MCP evidence. The response is less polished than requested, but it does not lie. Do not trade that truth boundary for a smoother answer.
Next: W-R3 is closed. Remain on the Ring lane and observe W-R4 only when the channel owner supplies a real ingress turn; no Phylax, Zenod, or Calli changes are authorized here.
