...
- Stateless reconnect continues successfully; a true child-process restart rejects the old signing key with an explicit restart error. A foreign vault with identical fixture bytes rejects the cursor. Forgery, changed order and altered evidence content fail visibly.
