...
CI on `b6098a4` identified two compatibility failures: normal multi-sentence answer whitespace was normalized even when no absence clause was removed, which also prevented original streaming chunk replay. The correction returns original text byte-for-byte when nothing is suppressed. Existing engine assertions remain unchanged; an explicit unchanged-byte helper regression was added. No behavior or policy expansion.

Corrected local validation: 98 core tests PASS, including the complete engine suite and both previously failing cases; 14 authenticated customer tests PASS; core typecheck PASS. Exact new-head CI and independent delta re-review remain integration gates.
