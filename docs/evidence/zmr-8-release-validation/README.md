...
Initial repair `624a2b2` selected memory finalization from memory tool attempts, citation claims or exhaustive requests. Independent review found a P2 bypass: an explicit question about saved memories could receive and persist invented text if the model omitted both tools and citations. The correction below closes that recognizable request boundary; the initial green CI does not validate the corrected source. The inherited source grounding heuristic is not a universal natural-language entailment proof. Remote models, live browser journeys and rollout gates remain unmeasured/unexecuted. Independent review and exact-head CI are required before integration.


### Independent-review correction: explicit memory requests require grounding

The host now recognizes explicit personal-memory requests on the vault-backed customer path before considering model tool activity. Requests such as “What payroll provider did I save in my memories?”, “Search my notes for the payroll provider”, and “What did I previously save about payroll?” therefore require the same shared source finalizer even when the model calls no tool and returns `readPaths: []`. This is a conservative explicit-request rule, not a universal semantic-intent classifier. Generic conversation and general explanations such as computer RAM are preserved; vaultless peer agents retain their connected-source contract.

Each of the three new phrase regressions exercises two authenticated streaming turns and two nonstreaming turns, rejects the unsupported claim from deltas/final replies, and checks the following turn's assistant history. A general-memory explanation regression ensures ordinary conversation still passes. Existing assertions remain intact. Corrected-source public ZMR tests: **21/21 PASS**, including **12/12** customer HTTP cases; all-workspace typecheck PASS. The focused core compatibility suite also passes **101/101** on this correction. Fresh exact-head CI and independent re-review remain required before integration; real-model and live acceptance are unchanged gates.
