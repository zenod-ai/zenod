...
## Independent review and CI correction — 2026-09-06

Initial candidate `2617e0273439861eb2ee34d581253c340407b120` was not integrated. Required CI run `34034214736` failed exactly two existing Drive repository tests: their scripted answer stubs returned `readPaths` without calling a read tool. Both failures reproduced locally. The stubs now read/assert the real Drive-backed renewal/Home passage and provider identity, returning empty model `readPaths` so host-successful reads remain the authority. This is a test-contract correction, not a rollback of the grounding guard or a transient-CI rerun.

Independent review also found P2: the coverage tracker treated any bootstrap-pinned ref as complete for a later catalog, although its body could change between pin resolution and the first search. A stable final catalog snapshot did not detect that earlier change. The unversioned-pin shortcut is removed. Pinned-only answers retain their primary resolved context; a catalog audit requires fresh complete post-enumeration reads for selected refs, including pins not tied to that snapshot.

The new regression failed before the fix in both the coverage tracker and public Drive MCP. Public provider fixtures now change the synthetic body after pinned bootstrap and before the first catalog search: a no-reread answer remains partial with the exact ref unread, while a fresh exact body read permits completion. Existing pinned current/previous followups still pass. The test-only body is restored between cases; no live memory changed.

Correction validation: core build and server typecheck pass; public typed Q&A, passage and MCP tests **35/35** pass. The complete Drive repository and coverage-suite result, corrected SHA and renewed CI/review state are recorded in the issue handoff. No worker merge or deployment.
