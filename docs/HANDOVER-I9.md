...
- **STEP 4 · C-17 budget kill — PASS (live-fire), with a real bug found + fixed:**
  - Landed the per-run budget override as the **B1 mechanism** (not scaffolding): `parseRunBudget` reads `budget {minutes,turns}` from task context (both "3 min / 10 turns" and structured "minutes: 3" forms) → env fallback. Verified live: run entry showed `budgetMs=180000, budgetTurns=10`.
  - **Live-fire caught a wiring bug the unit tests missed** (`6559e87`): `activeHeartbeatRuns` rebuilt each run WITHOUT `pid`/`budgetMs`/`budgetTurns`, and the sweep flagged `budgetKill` on the reshaped copy not the state entry — so the budget kill could never fire *or* render. First runaway blew past the ceiling ("File 21…") and kept going. Fixed both; added a regression test asserting `activeHeartbeatRuns` preserves pid + budget. (The lesson, logged: `budgetKillDecision` was unit-tested in isolation; the sweep integration was not.)
  - **Clean retest PASS** (`ephemeral-1783126896084-938aae9e`): journal `{launch}`→`{budget-kill "turn budget exceeded: 41 > 10 turns"}`; monitor log *"budget kill … terminating pid 846"*; WhatsApp: `⛔ … failed: budget exceeded … Terminated by the run budget (C-17) — nothing verifiable` + transcript link. No manual intervention.
