...
### 2026-07-05 · STT follow-up — OpenRouter topped up + verified; watchdog-generalization ticket

- **OpenRouter top-up VERIFIED (no code, no deploy — freeze holds).** Re-ran the transcribe live-fire against `c1.zenod.dev` (066835c), same method (real TTS clip → `POST /api/chat/voice/transcribe`, bearer-authed): **HTTP 200**, `provider: "openrouter openai/whisper-large-v3-turbo"` — cloud STT primary is back (no 402, did not fall to whisper). Server log: `[voice] web transcribe OK: provider=openrouter openai/whisper-large-v3-turbo`. **#594 CLOSED** with this receipt; whisper (#592) remains the tested fallback for the next credit dip.
- **Ticket filed — #596: "Watchdog: headroom checks for ALL metered external dependencies (generalize C-25)."** Post-soak batch. Generalizes C-25 beyond the claude engine to every metered external key (OpenRouter `/credits` balance first; registry for Groq/OpenAI/Composio/future), polled by the existing watchdog with warn/page thresholds and the two-tier Phylax alert. Evidence: #594's 402 killed cloud STT silently — found by accident, not by alert. (Typed `create_issue` front door routed it into Epaminon's execution lane without a synchronous receipt — the #589/#590 ack-before-receipt gap again — so it was filed directly via gh for a verifiable URL; any lane-produced duplicate should be closed.)
