...
| 2026-07-10 | D20 (POSTMORTEM — spine-authoring laws, binding on this planner for every future spine; full record: epicspine issue #6): the 2026-07-10 day-long stall was caused by how the PLANNER authored these spines, in five ways: maximal wish-list DoDs; proof-before-demo ("no human in the loop" — banned language from now on); autonomy without time budgets; uniform maximum ceremony regardless of risk class; co-development without sync rules. Corrective laws — all serving ONE metric, time-until-Jordi-tests-the-finished-thing (Jordi 2026-07-10: "the objective is me being able to test it being done"; not Jordi-in-the-loop-per-increment, and testing itself is NOT the problem — workers always test before handover; the banned thing is any work that does not shorten this metric: parallel harness worlds, re-proofs of proven commits, ceremony on dead assets, unrequested evidence): (A) Minimum Lovable Done — every DoD splits into SHIP (3–5 items, ends with a worker-tested deployment handed to Jordi) and HARDEN (follow-up tickets after Jordi has tested SHIP); (B) SHIP's done-condition is Jordi testing a thing the worker already tested; (C) every ticket carries a time box — on expiry, stop and report options; (D) ceremony proportional to risk class (live-customer data = full; test/dead/reversible = snapshot-and-go; one drill proves a mechanism epic-wide); (E) base pin + lap limit + provider quiesce for co-development. Remaining 3.x work and all Phase 2 spines are re-authored under these laws before dispatch. | The product and design were fine all day; the process the planner wrote burned the hours. Accountability recorded; laws encoded here and in the public skill so they outlive this session. | https://github.com/AlfaBlok/epicspine-skill/issues/6; D19 |
| 2026-07-10 | D21 (THE TEST THAT MATTERS — supreme over all other validation): for every unit, THE acceptance test is the worker driving a real browser through the full customer journey on the LIVE deployment — for Zenod: public buy page → Stripe TEST checkout → tenant auto-created → customer-style login/claim → Keys & models works → key entered → ingest → receipt visible. The worker runs this journey IN A LOOP, fixing whatever breaks each lap, until it completes clean — then hands Jordi the starting URL to repeat it. AUTH PLANES (correction 2026-07-10, Jordi): the customer journey uses the REAL human auth — claim link → GitHub OAuth → hosted-entry → console session (the existing 2.x flow, preserved in the chassis); paste-token login must never appear as the customer front door. Tokens are exclusively the AGENT credential (`/mcp/<token>`, ZD-8), surfaced inside the console for wiring agents. Unit/integration tests are supporting material run behind the journey, never the definition of done; no amount of green suites means anything while the journey is broken. Evidence = the journey's browser screenshots, nothing more. | 2026-07-10: 1,100+ green tests coexisted with a blank Keys page and a missing checkout→tenant handoff; Jordi found both in sixty seconds of clicking. The journey is what customers buy; it is therefore what "working" means. | Jordi 2026-07-10; supersedes the emphasis (not the content) of the Autonomous Validation Protocols in 3.1–3.6 |
