...
Last reconciled commit: `f2f5db946d182fa7e1f244d7bcb7b14e3145ff72` on `main`
...
Context: read-only Dokploy preflight found the public Hosted service healthy but on old source SHA `bcd64987`, with no Google OIDC/Drive vault configuration keys. It also exposed a source mismatch: production readiness expected a host-managed Drive OAuth client pair, while tenant runtimes used `seedFromEnv:false` and required each tenant to persist its own client credentials. That contradicted the approved Hosted onboarding and would have made a real consent test fail even after configuring Dokploy.

Action: created #1182 and merged [PR #1183](https://github.com/zenod-ai/zenod/pull/1183) as `f2f5db946d182fa7e1f244d7bcb7b14e3145ff72` after exact head `fd9db0b9fcfa006050493d64802b3a5276d028dd` passed full CI and clean independent review. The process-level pair is captured once, available only to eligible active Hosted tenants, hidden from tenant settings and independent of tenant-local archive/vault refresh tokens, bindings and caches. No live secrets, provider calls or deployment changed.

Next: choose and approve the credential-backed target. The recommended path is a new isolated `cloud-test.zenod.dev` Dokploy application with an empty volume, Stripe test mode, public signup closed and one disposable allowlisted Google user. Direct production rollout remains a separate higher-risk option requiring a fresh off-host backup and explicit production/customer approval. Either path still requires exact Google OIDC and Drive OAuth clients/callbacks/branding/scopes.

Risks: no staging application currently exists, production is on old SHA `bcd64987`, and production has none of the required Google OIDC/Drive vault/readiness keys. Creating staging is a new external application and requires Jordi's permission; deploying directly to production risks existing tenants and cannot be inferred from a general request to test the feature.
