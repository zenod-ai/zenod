...
Phase: Recoverable Drive + Git-bundle backend complete; GDV-7 tenant runtime ready
Last verified: 2026-08-29 CEST
Integration target: `main`
Fresh base commit: `16605cf455f760b5e0f0e453a663e05e979b52d5` on `main`
...
Next action: dispatch GDV-7 from exact fresh `main` `16605cf` to bind existing Drive credentials, provider selection and tenant runtime to the reviewed `DriveVaultRepository`, without UI, live credentials, deployment or production changes.
...
| [GDV-7 #1151](https://github.com/zenod-ai/zenod/issues/1151) | Ticket worker | unassigned | Bind Drive credentials, backend selection and tenant runtime | ready | #1146, #1147, #1150 | `codex/gdv-7-drive-tenant-runtime` | `16605cf` on `main` | Paid Drive-only tenant provisions, reconstructs and fails closed on revoked consent without GitHub. | Identity, OIDC, provider-neutral runtime and reviewed Drive backend are integrated. | 2026-08-29 | Dispatch one ticket worker from exact fresh main; keep credentials/deploy behind human gates. |
...
1. Dispatch GDV-7 on `codex/gdv-7-drive-tenant-runtime` from exact `main` `16605cf`.
2. Independently prove one paid Drive-only tenant provisions/reconstructs through the reviewed backend, remains tenant-isolated and fails closed on revoked or incomplete consent.
3. After GDV-7 integrates, dispatch GDV-8 and GDV-9 as the dependency graph permits; keep live OAuth/config/deployment behind human gates.
