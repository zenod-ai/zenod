...
**`archus.enqueue_execution`** — Archus → Epaminon, when Archus mints an execution ticket.
```
...
- `needsRepo: false`; drop all backlog-write tools. Expose `enqueue_execution` (receive a
  dispatch) and `execution_status` (human read).
- Loop: pull from queue under a concurrency limit → launch the runner/Codex → on each edge
  call `apply_execution_event` → on a PR set `needs-review` → fan-in/integrate in order →
  on a blocker, try context first, then one `chat_with_archus` unblock call.
...
Human:                 approves merge → runner merges on green
Epaminon → Archus:     apply_execution_event(E, done, pr_url)            [deterministic]
Archus:                reflects outcome onto backlog #5
```
