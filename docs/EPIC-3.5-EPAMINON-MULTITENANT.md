...
| Epic worker | unassigned | This spine | Delivery lead: split Epaminon into multi-tenant API + per-job sandboxes. | Spine, issues, integration state current. |
...
- [ ] Two-tenant smoke test: tenant A cannot see/cancel/read tenant B's jobs; sandbox of A holds no B material.
...
Next action: wait for 3.1; decide sandbox spawner mechanism (Docker socket vs host runner — parent open question 3).
Blockers: depends on Epic 3.1; spawner decision pending (Jordi).
...
Ticket sketch: E-MT-1 API on chassis + tenant-scoped job store; E-MT-2 spawner seam + chosen mechanism; E-MT-3 pinned worker image + job env contract; E-MT-4 lifecycle/orphan-reaping + crash tests; E-MT-5 quotas/metering/fairness; E-MT-6 two-tenant smoke + self-host parity; E-MT-7 remove AWAIT_PROVISION and retire per-user instances.
