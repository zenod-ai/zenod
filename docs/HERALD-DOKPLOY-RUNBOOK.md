# Herald Dokploy runbook

This is the guarded zero-state recipe for the one new Herald application. It is
plan-first and must not be applied until H-S1 is integrated into `main`, the
immutable `sha-*` image exists, and the Herald delivery manager supplies the
...
Ring, Zenod, or Callisthenes applications. Record the new Herald `applicationId`.

The script reads OAuth and Stripe TEST values from the shipped Ring
application through the Dokploy API, keeps them in memory, and sends only the
allowlisted Herald environment to the new target. It never prints values or
...
```sh
eval "$(dokploy-env)"
TARGET_APP_ID=<new-herald-application-id> \
EXPECTED_SHA=<integrated-main-sha> \
STATE_DIR=/var/tmp/h-s1-cutover-$(date -u +%Y%m%dT%H%M%SZ) \
scripts/h-s1-dokploy-cutover.sh
```
...
APPROVAL_REF=issue-897-manager-cutover \
TARGET_APP_ID=<new-herald-application-id> \
EXPECTED_SHA=<integrated-main-sha> \
STATE_DIR=/var/tmp/h-s1-cutover-<same-recorded-timestamp> \
scripts/h-s1-dokploy-cutover.sh
```

Apply updates and deploys only the new Herald target, attaches only
`herald.zenod.dev`, and requires a receipt for `/`, `/app`, `/mcp`, `/healthz`,
...
`BLOCKED ON JORDI: add https://herald.zenod.dev/auth/github/callback to the shared GitHub OAuth app.`
...
APPROVAL_REF=issue-897-manager-rollback \
TARGET_APP_ID=<new-herald-application-id> \
EXPECTED_SHA=<integrated-main-sha> \
STATE_DIR=/var/tmp/h-s1-cutover-<same-recorded-timestamp> \
scripts/h-s1-dokploy-cutover.sh
...
Rollback removes only the new Herald domain. The new target and its volume stay
