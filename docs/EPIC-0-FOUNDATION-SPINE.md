...
Fresh base commit: `8d62a6f197fe7ed836eb93ff9c6c8569c6ccc95e` on `main`; the reviewed single €9/month contract is integrated and [ZAL-19 #1092](https://github.com/zenod-ai/zenod/issues/1092) is the final bounded public-copy correction
Next action: complete and independently review ZAL-19 while resolving the exact credential, €9 Stripe, backup, and Dokploy inputs read-only. After ZAL-19 integrates, publish one immutable final image, refresh ZAL-4 for that digest, take verified two-volume backups, deploy with signup closed, and run the live release-candidate acceptance before handing Jordi the final test.
Blockers: GitHub Actions reports a major outage; the earlier `ba5e9c1` publish retry remains queued and no digest may be inferred. The final deployment will instead use the post-ZAL-19 merge image. Exact off-host backup destination/procedure, Google credential source, shared Channels token, live €9 Stripe price, and reviewed non-secret Dokploy delta are being resolved. Public signup remains closed until Jordi's final experiential test and separate opening decision.
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| `docs/EPIC-ZENOD-ALPHA-LAUNCH.md` | Epic worker | Zenod Alpha delivery manager | Deliver trustworthy Zenod alpha | running / final candidate preparation | ZAL-19, final immutable image, exact production inputs, backups, closed deploy and acceptance | [ZAL-19 #1092](https://github.com/zenod-ai/zenod/issues/1092); [ZAL-18 PR #1091](https://github.com/zenod-ai/zenod/pull/1091) merged | `8d62a6f` | Child DoD covers readiness truth, recap regression, the approved one-plan offer, fail-closed production gate, stranger acceptance, promotion draft, and resumable state. | ZAL-18 is green/integrated; ZAL-19 is dispatched as the final copy-only correction; production remains unchanged and signup closed while exact operations inputs are reconciled. | 2026-08-26 CEST | Integrate ZAL-19, publish its immutable image, refresh ZAL-4, then execute the reviewed closed-candidate sequence through final Jordi test. |
