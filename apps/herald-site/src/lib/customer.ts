...
export const PRICING_OPTIONS: ReadonlyArray<{
...
}> = [
  {
    ...
    price: "Free",
    cadence: "forever",
    description: "Run Herald on your own infrastructure with your own model, memory, and publishing credentials.",
    tier: null,
  },
  {
    name: "Monthly",
    price: "€5",
    cadence: "per month",
    description: "Hosted Herald with tenant-scoped chat, briefing, board, wallet, and receipts.",
    tier: "monthly",
  },
  {
    name: "Yearly",
    price: "€50",
    cadence: "per year",
    description: "The same hosted Herald loop with two months included in the annual price.",
    tier: "yearly",
  },
]
...
export async function readCustomerSession(
  ...
  if (!response.ok) throw new Error("Could not read your Herald session")
  ...
  if (typeof payload.login !== "string" || payload.login.length === 0) {
    throw new Error("Herald returned an invalid session")
  }
...
export async function createHostedCheckout(
  ...
  const response = await fetcher(CHECKOUT_PATH, {
    ...
    body: JSON.stringify({ product: "herald", unit: "herald", tier }),
  })
