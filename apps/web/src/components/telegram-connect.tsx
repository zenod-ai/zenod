...
export function HostedTelegramConnect({
  ...
  const controlState: ChannelState = state ?? "loading"
  ...
  return (
    <ChannelControlCard
      control={{
        ...
        label: "Telegram",
        state: controlState,
        identityHint: channels?.telegram.identityHint
          ? state === "awaiting_code"
            ? `Pending identity ${channels.telegram.identityHint}.`
            : `Linked identity ${channels.telegram.identityHint}.`
          : null,
        description: "Telegram reaches the same Zenod memory directly.",
      }}
    ...
    >
      ...
      {state === "awaiting_code" && challenge && (
        <Alert>
          ...
          <AlertDescription>
            Message <strong>{challenge.code}</strong> to the Zenod Telegram bot
            from {channels?.telegram.identityHint ?? "that identity"}. Zenod
            activates only after that exact identity replies.
          </AlertDescription>
        </Alert>
      )}
      {state === "awaiting_code" && !challenge && (
