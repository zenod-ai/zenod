...
export const PRICING_OPTIONS: ReadonlyArray<{
...
}> = [
  {
    ...
    price: "Free",
    cadence: "forever",
    description: "Run Callisthenes on your own infrastructure with your own X credentials.",
    tier: null,
  },
  {
    name: "Monthly",
    price: "€5",
    cadence: "per month",
    description: "Hosted Callisthenes with tenant-scoped X custody, drafts, throttling, and receipts.",
    tier: "monthly",
  },
  {
    name: "Yearly",
    price: "€50",
    cadence: "per year",
    description: "The same hosted Callisthenes with two months included in the annual price.",
    tier: "yearly",
  },
]
...
export async function readCustomerSession(
  ...
  if (!response.ok) throw new Error("Could not read your Callisthenes session")
  ...
  if (typeof payload.login !== "string" || payload.login.length === 0) {
    throw new Error("Callisthenes returned an invalid session")
  }
...
export async function createHostedCheckout(
  ...
  const response = await fetcher(CHECKOUT_PATH, {
    ...
    body: JSON.stringify({ product: "callisthenes", unit: "callisthenes", tier }),
  })
