...
export const PRICING_OPTIONS: ReadonlyArray<{
...
}> = [
  {
    ...
    price: "Free",
    cadence: "forever",
    description: "Run Ring on your own infrastructure with your own model and unit credentials.",
    tier: null,
  },
  {
    name: "Monthly",
    price: "€5",
    cadence: "per month",
    description: "Hosted Ring with a tenant-scoped council chat, unit wallet, keys, and receipts.",
    tier: "monthly",
  },
  {
    name: "Yearly",
    price: "€50",
    cadence: "per year",
    description: "The same hosted Ring with two months included in the annual price.",
    tier: "yearly",
  },
]
...
export async function readCustomerSession(
  ...
  if (!response.ok) throw new Error("Could not read your Ring session")
  ...
  if (typeof payload.login !== "string" || payload.login.length === 0) {
    throw new Error("Ring returned an invalid session")
  }
...
export async function createHostedCheckout(
  ...
  const response = await fetcher(CHECKOUT_PATH, {
    ...
    body: JSON.stringify({ product: "ring", unit: "ring", tier }),
  })
