...
const PENDING_TIER_KEY = "ring.pending-checkout-tier"
  ...
  return (
    <header className="border-b border-border bg-background">
      <div className="mx-auto flex min-h-16 max-w-6xl items-center justify-between px-5">
        <a href="/" className="font-display text-xl font-bold tracking-tight">RING</a>
  ...
  return (
    <section id="pricing" className="border-t border-border px-5 py-20">
      <div className="mx-auto max-w-6xl">
        ...
        <h2 className="font-display mb-10 text-4xl font-semibold">Bring your council together.</h2>
          ...
          {PRICING_OPTIONS.map((plan) => (
            <article key={plan.name} className="flex min-h-72 flex-col bg-background p-7">
              ...
              {plan.tier ? (
                ...
                <Button asChild variant="outline"><a href="https://github.com/zenod-ai/zenod/tree/main/units/ring">Self-host</a></Button>
              )}
            </article>
          ))}
        </div>
    ...
    <>
      ...
      <main>
        <section className="px-5 py-24 sm:py-32">
          <div className="mx-auto max-w-6xl">
            <p className="label-caps mb-5 text-rust">One council for all your agents</p>
            ...
            <p className="mt-7 max-w-2xl text-lg leading-8 text-muted-foreground">
              Ring gives you one place to think with your council, connect the units you already
              trust, and see the receipts when work is committed.
            </p>
        ...
        <section className="border-t border-border px-5 py-16">
          <div className="mx-auto grid max-w-6xl gap-px bg-border md:grid-cols-3">
            {["One chat with your council", "Your units stay in your wallet", "Commit receipts in the conversation"].map((item) => (
      ...
      <footer className="border-t border-border px-5 py-8 text-sm text-muted-foreground">
        <div className="mx-auto flex max-w-6xl items-center justify-between"><span>Ring · AGPL-3.0</span><span className="flex items-center gap-2"><CheckIcon className="size-4" />GitHub-only identity</span></div>
      </footer>
    </>
  )
}
