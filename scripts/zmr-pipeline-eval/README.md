...
Both drivers accept `--organizer-reasoning-effort none` (or `low`), forwarded through the real adapter's `organizerReasoningEffort` option; this is not a transport rewrite. The server uses the corresponding tenant setting `model_classify_reasoning_effort`, seeded by `ZENOD_MODEL_CLASSIFY_REASONING_EFFORT`. Omission preserves provider defaults. Only explicit `none` or `low` on OpenAI/OpenRouter is accepted. Classification, reconciliation and backlog extraction receive the setting; answers and whole-page composition do not, even when they share the same model ID. Existing output caps and schema validation remain unchanged.

The installed OpenAI SDK serializes this as `reasoning_effort` with the selected value. [OpenRouter reasoning documentation](https://openrouter.ai/docs/guides/best-practices/reasoning-tokens) describes effort as a reasoning allocation control, not a hard token budget; excluding reasoning text does not save reasoning tokens. On 2026-09-14 the [public model catalog](https://openrouter.ai/api/v1/models) listed DeepSeek V4.1 Flash with default effort `high`, supported effort `low`, and optional reasoning. A manager-dispatched replay of one previously truncated request with `none` returned valid JSON in 19.94 seconds: 1094 completion tokens, zero reported reasoning tokens, and $0.001839 reported cost on the Together route. This is one compatibility result, not provider-wide reliability or semantic acceptance. The option stays explicit; no silent fallback or token-cap increase occurs. Offline wire tests prove stage-isolated forwarding; full structured completion and semantic quality still require bounded live acceptance.
...
Organizer route compatibility can be evaluated with `--organizer-provider-order fireworks,together` (example only, not a availability recommendation). This forwards the real `organizerProviderOrder` adapter option: one to three unique lowercase base provider slugs, ordered and allowlisted via OpenRouter `provider.only`/`provider.order`, with `require_parameters: true`. No unlisted route, model fallback, endpoint tier variant, or priority/nitro selector is added. Omission preserves gateway defaults; ask and compose routing are unaffected even when model IDs match. Provider support, availability and cost require separate actual evaluation; syntactically valid routing is not a quality or price guarantee. See [official provider routing](https://openrouter.ai/docs/guides/routing/provider-selection).

The equivalent hosted setting is `model_classify_provider_order`, seeded by `ZENOD_MODEL_CLASSIFY_PROVIDER_ORDER`, containing the same comma-separated slugs. Empty deletes the override. It is OpenRouter-only and follows the existing tenant settings/runtime invalidation path. Explicit routing does not bypass the evaluation transport guard: the SDK adapter delegates to the current global fetch after adding the provider envelope, preserving request model, caps, reasoning and schema fields.

A request deadline aborts transport and latches `INCOMPLETE_PROVIDER_DEADLINE`. Later SDK/job attempts and remaining trials cannot send another request. A late response cannot settle usage or overwrite evidence after the deadline; its reservation remains unknown exposure. After cleanup and the final private receipt are written, the driver exits nonzero even if the transport ignores cancellation. This does not prove that upstream computation stopped or that the request was free.
