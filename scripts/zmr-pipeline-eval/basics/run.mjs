...
const run={suite:fixture.version,candidateSha:a['candidate-sha'],observedDeployedSha:a['deployed-sha'],deploymentIdentity:'operator-supplied; this isolated runner does not verify live health',mode:a.live?'ACTUAL_ENGINE_CHAT':'OFFLINE_PREFLIGHT',fixtureSha256:sha256(fixtureBytes),rubricSha256:sha256(rubricBytes),pricingSha256:sha256(pricingBytes),models,startedAt:new Date().toISOString(),limits:{budgetUsd:Number(a['budget-usd']),maxRequests:Number(a['max-requests']),requestDeadlineMs:120000,plannedOutcomes:rows.length},plannedKeys:rows.map(r=>r.key),selected:selected.map(r=>r.key),outcomes:rows,rubricCheckCounts:Object.fromEntries(rubric.cases.map(c=>[c.id,c.checks.length])),requests:ledger.rows,budgetBlocks:[],externalCalls:0,usage:[],sourceHashes:await hashes(),harnessHashes:{},compiledHashes:{},status:'RUNNING'};
for(const rel of ['run.mjs','policy.mjs','background.mjs','report.mjs',suiteFiles[0],suiteFiles[1],'../policy.mjs','../models.mjs'])run.harnessHashes[rel]=sha256(await readFile(new URL(rel,import.meta.url)));
