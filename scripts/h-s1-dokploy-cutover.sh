...
# H-S1 duplicates the guarded Z-N5/C-S4 recipe for a NEW Herald application.
# The shipped Ring application is read only and supplies only allowlisted OAuth and
...
STATE_DIR="${STATE_DIR:-/tmp/h-s1-dokploy-cutover-state}"
...
RING_SOURCE_APP="hkdStWh6zfJ9d-uohdJHt"
HERALD_HOST="herald.zenod.dev"
...
[[ "$TARGET_APP_ID" != "$RING_SOURCE_APP" ]] || die "target must be a new Herald application; protected Ring id refused"
...
build_target_env() {
  ...
  jq -nr --arg target "$(jq -r '.env//""' <<<"$target")" \
    ...
    parsed($target) as $t|parsed($source) as $s|{
      ...
      CUSTOMER_APP_URL: "https://herald.zenod.dev",
      DOMAIN: "https://herald.zenod.dev",
      GITHUB_OAUTH_CALLBACK_URL: "https://herald.zenod.dev/auth/github/callback",
      ...
      ZC_COOKIE_DOMAIN: "herald.zenod.dev",
      ZENOD_DATA_DIR: "/data",
      ZENOD_SITE_DIST: "/app/apps/herald-site/dist",
      ZENOD_UNIT: "herald",
...
attach_domain() {
  local domains="$1"
  [[ -z "$(domain_id "$domains" "$HERALD_HOST")" ]] || { log "domain already attached"; return; }
  api_post /domain.create "$(jq -n --arg host "$HERALD_HOST" --arg applicationId "$TARGET_APP_ID" \
    '{host:$host,path:"/",port:8080,https:true,certificateType:"letsencrypt",domainType:"application",applicationId:$applicationId}')" \
    "attach $HERALD_HOST to Herald application port 8080"
}
...
final_world_ready() {
  ...
  [[ "$(http_code "https://$HERALD_HOST/")" == 200 ]] || return 1
  [[ "$(http_code "https://$HERALD_HOST/app")" =~ ^(200|302|303)$ ]] || return 1
  health="$(curl -fsS "https://$HERALD_HOST/healthz")" || return 1
  [[ "$(jq -r '.status // empty' <<<"$health")" == ok ]] || return 1
  runtime_health="$(curl -fsS "https://$HERALD_HOST/api/health")" || return 1
  ...
  mcp="$(curl -sS -o /dev/null -w '%{http_code}' -X POST -H 'Content-Type: application/json' \
    --data '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}' "https://$HERALD_HOST/mcp")"
  [[ "$mcp" == 401 ]] || return 1
  oauth="$(curl -sS -o /dev/null -D - "https://$HERALD_HOST/auth/signin" | awk 'BEGIN{IGNORECASE=1} /^location:/{sub(/\r$/, ""); print substr($0,11)}')"
  [[ "$oauth" == https://github.com/login/oauth/authorize* ]] || return 1
  [[ "$oauth" == *"redirect_uri=https%3A%2F%2Fherald.zenod.dev%2Fauth%2Fgithub%2Fcallback"* ]] || return 1
}

verify_world() {
  [[ "$DRY_RUN" == 0 ]] || { log "DRY_RUN verify / 200, /app 200|redirect, /healthz exact SHA, /mcp 401, OAuth callback=Herald"; return; }
  local deadline=$((SECONDS+HEALTH_TIMEOUT_SECONDS))
  until final_world_ready; do
    (( SECONDS < deadline )) || die "Herald health receipt did not converge; rollback with the same STATE_DIR"
    sleep "$HEALTH_POLL_SECONDS"
  done
  jq -n --arg url "https://$HERALD_HOST" --arg sha "$EXPECTED_SHA" --arg at "$(date -u +%FT%TZ)" \
    '{url:$url,sha:$sha,verified_at:$at,routes:{landing:200,app:"200|redirect",mcp:401,healthz:200},oauth_callback:"https://herald.zenod.dev/auth/github/callback"}' >"$STATE_DIR/health-receipt.json"
  chmod 600 "$STATE_DIR/health-receipt.json"
}

apply_cutover() {
  local target source domains env
  target="$(app_json "$TARGET_APP_ID")"; source="$(app_json "$RING_SOURCE_APP")"; domains="$(app_domains "$TARGET_APP_ID")"
  [[ "$(jq -r '.name' <<<"$target")" == herald ]] || die "target application must be named herald"
  ...
  [[ "$(env_value "$source" STRIPE_SECRET_KEY)" == *'_test_'* ]] || die "Ring source Stripe key is not TEST"
  env="$(build_target_env "$target" "$source")"
  log "PLAN 1/4 snapshot only the new Herald target; preserve /data"
  ...
  api_post /application.deploy "$(jq -n --arg applicationId "$TARGET_APP_ID" '{applicationId:$applicationId,title:"H-S1 guarded Herald deploy"}')" "deploy new Herald application"
  log "PLAN 3/4 attach only herald.zenod.dev; never detach or stop another service"
...
rollback() {
  ...
  domains="$(app_domains "$TARGET_APP_ID")"; id="$(domain_id "$domains" "$HERALD_HOST")"
  [[ -z "$id" ]] || api_post /domain.delete "$(jq -n --arg domainId "$id" '{domainId:$domainId}')" "remove Herald target domain"
  old_image="$(jq -r '.dockerImage' "$STATE_DIR/target.before.json")"
  log "rollback removes only the new Herald domain; target image remains $old_image for manager-directed recovery"
}

if [[ "${H_S1_SOURCE_ONLY:-0}" != 1 ]]; then
